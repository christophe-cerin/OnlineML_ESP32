...
int main() {
    ...
    for (int i = 0; i < N-1; ++i) {
        ...
        if (i == N-2) {
	     ...
	     - λ (l1)  Increase for more sparsity : 0.1 -> 5.0
	     - Learning Rate: Reduce if oscillations : 0.01 -> 0.001
	     */

            //OptimizedLasso model(0.01, 1000, 1.0);
            OptimizedLasso model(0.001, 1000, 5.0);
            ...
            plt::save("regressionLassoOptimizedHyperParaTunning.png");
            plt::show();
        }
