...
int main() {
    ...
    for (int i = 0; i < N-1; ++i) {
        ...
        if (i == N-2) {
            ...
            plt::title("Regression LASSO & Collinearity Check - Real Values & Predictions");
            plt::xlabel("X (standardized)");
            plt::ylabel("Y (standardized)");
            plt::grid(true);
            plt::legend();
            plt::save("regression_lasso_optimizedEngi.png");
            plt::show();

            // Residual Analysis
            VectorXd residuals = y_test - y_pred;
            vector<double> residuals_vec(residuals.data(), residuals.data() + residuals.size());
            
            plt::figure();
            plt::hist(residuals_vec, 30, "purple");
            ...
            double residual_std = sqrt((residuals.array() - mean_residual).square().sum() / residuals.size());
            
            cout << "\nResidual Analysis:" << endl;
            cout << "Mean residual: " << mean_residual << endl;
            cout << "Residual standard deviation: " << residual_std << endl;
            
            plt::save("residuals_analysisEngi.png");
            plt::show();
        }
