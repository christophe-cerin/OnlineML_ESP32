...
class DenStream():
    ...
    **Offline generation of clusters on-demand (clustering)**
    ...
    def _generate_clusters_for_labels(cluster_labels):
        ...
        # Group micro clusters per label
        for mc, label in cluster_labels.items():
            if label not in mcs_per_label:
                mcs_per_label[label] = []
            mcs_per_label[label].append(mc)

        # Process each label and merge micro clusters
        for label, micro_clusters in mcs_per_label.items():
            # Initialize the cluster using the first micro cluster
            cluster = copy.copy(micro_clusters[0])
            
            # Merge remaining micro clusters
            for mc in range(1, len(micro_clusters)):
                cluster.merge(micro_clusters[mc])

            # Store the merged cluster for the label
            clusters[label] = cluster
...
def build_denstream(decaying_factor=0.05, beta=0.5):
    return DenStream(
        decaying_factor=0.4,
        beta=0.75,
        mu=2.5,
