...
if __name__ == "__main__":
    ...
    y = 1 + 2 * np.sin(X)
    yhat = y + 0.5 * np.random.normal(size=len(X))
    ...
    # Plot unregularized regressor
    r.fit(tX, y)
    plt.plot(X, r.predict(tX), "b", label="ŷ (alpha=0.0)")

    # Plot regularized regressor
    alpha = 3.0
    r.fit(tX, y, alpha)
    plt.plot(X, r.predict(tX), "y", label="ŷ (alpha=%.1f)" % alpha)
