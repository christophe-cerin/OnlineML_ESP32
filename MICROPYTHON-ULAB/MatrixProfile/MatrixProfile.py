    ...
    for k in range(s):
        ...
        D = (
            ...
            * (
                1
                - (
                    (product_me - ((sumQuery * sumTarget) / m))
                    / np.sqrt(
                        (sumSquareQuery - ((sumQuery**2) / m))
        ...
        for i in range(1, s - k + 1):
            ...
            sumSquareTarget = (
                sumSquareTarget - data[kplusi - 1] ** 2 + data[kplusi + m - 1] ** 2
            )

            product_me = (
                product_me
                - (data[i - 1] * data[kplusi - 1])
                + (data[i + m - 1] * data[kplusi + m - 1])
            )
...
def AAMP_ned(X, m):
    ...
    for k in range(s):
        ...
        for i in range(1, s - k + 1):
            ...
            D = (
                D
                - (X[i - 1] / mm - X[kplusi - 1] / mm) ** 2
                + (X[i + m - 1] / mm - X[kplusi + m - 1] / mm) ** 2
            )
            if Dmin[i] > D and matchFlag:
                minind[i] = kplusi
                Dmin[i] = D
...
if __name__ == "__main__":
    ...
    aamp_ned_norm = (mindist_ned - np.min(mindist_ned)) / (
        np.max(mindist_ned) - np.min(mindist_ned)
    )
    ...
    aamp_md_norm = (mindist_md - np.min(mindist_md)) / (
        np.max(mindist_md) - np.min(mindist_md)
    )
    ...
    mp_stump = (
        1.6 * matrix_profile[:, 0]
    )  # See https://stumpy.readthedocs.io/en/latest/api.html#stumpy.stump
