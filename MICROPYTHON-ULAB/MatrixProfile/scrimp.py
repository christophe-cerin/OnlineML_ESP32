...
Source: https://raw.githubusercontent.com/target/matrixprofile-ts/refs/heads/master/matrixprofile/scrimp.py

See also: https://github.com/target/matrixprofile-ts/tree/master/matrixprofile
and https://www.cs.ucr.edu/~eamonn/MatrixProfile.html
...
def test_scrimp_plus_plus():
    ...
    m = 4
    step_size = 0.25
    mp, mpidx = scrimp_plus_plus(ts, m, step_size)
    ...
    ts = np.loadtxt(os.path.join('.', 'sampledata.txt'))
    m = 32
    step_size = 0.25
    mp, mpidx = scrimp_plus_plus(ts, m, step_size)
    print('Matric profixe:',mp)
    print('Mpidx:',mpidx)
    
    #expected_mp = np.loadtxt(os.path.join(MODULE_PATH, '..', 'tests', 'mp.txt'))
    expected_mp = np.loadtxt(os.path.join('.', 'mp.txt'))

    np.testing.assert_almost_equal(mp, expected_mp, decimal=4)

#if __name__ == "__main__":
#    test_scrimp_plus_plus()
