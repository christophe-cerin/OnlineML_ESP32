    ...
    for j in range(0, Nb, my_size):
        ...
        for k in range(s):
            ...
            for i in range(1, s - k + 1):
                ...
                sumQuery = (
                    sumQuery
                    - np.array(ring.get()[i - 1])
                    + np.array(ring.get()[i + m - 1])
                )
                sumTarget = (
                    sumTarget
                    - np.array(ring.get()[kplusi - 1])
                    + np.array(ring.get()[kplusi + m - 1])
                )

                sumSquareQuery = (
                    sumSquareQuery
                    - np.array(ring.get()[i - 1] ** 2)
                    + np.array(ring.get()[i + m - 1] ** 2)
                )
                sumSquareTarget = (
                    sumSquareTarget
                    - np.array(ring.get()[kplusi - 1] ** 2)
                    + np.array(ring.get()[kplusi + m - 1] ** 2)
                )

                product_me = (
                    product_me
                    - (np.array(ring.get()[i - 1]) * np.array(ring.get()[kplusi - 1]))
                    + (
                        np.array(ring.get()[i + m - 1])
                        * np.array(ring.get()[kplusi + m - 1])
                    )
...
if __name__ == "__main__":
    ...
    st = time.time()
    [mindist_acamp, minind_acamp] = ACAMP_1(
        x_norm[0 : chunk_size * my_size], my_size, mm, ringbuffer
    )
    # get the end time
    et = time.time()
    ...
    plt.plot(
        x_norm[(chunk_size - 1) * my_size : chunk_size * my_size], "red", label="Input"
    )
