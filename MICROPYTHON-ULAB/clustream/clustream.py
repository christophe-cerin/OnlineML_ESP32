...
class CluStreamMicroCluster:
    ...
    def __init__(
        ...
        # Compute variance for each key in x
        for k in x:
            self.var_x[k] = self.compute_variance(x[k], w)

        # Compute variance for timestamp
        self.var_time = self.compute_variance(timestamp, w)

    def compute_variance(self, values, weight):
        # Compute variance manually
        if not values:
            return 0.0

        sum_x = sum(values)
        sum_x2 = sum(value ** 2 for value in values)
        count = len(values)
...
def build_clustream(decaying_factor=0.05, beta=0.5):
    return CluStream()

X_dicts = [dict(enumerate(x)) for x in X]

# Create an instance of Denstream
denstream = build_clustream()
...
for i, x in enumerate(X_dicts):
    ...
    print(f'[{X[i]}, {denstream.predict_one(x)}],')
